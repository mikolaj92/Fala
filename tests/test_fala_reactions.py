...
from fala.reactions import (
    FileReactionStore,
    MemoryReactionStore,
    content_address_file,
    content_address_json,
    digest_from_fala_reaction_uri,
    local_path_from_uri,
    resolve_reaction_local_path,
    sha256_digest,
)
from fala.models import ReactionRef


class ResolveReactionLocalPathTests(unittest.TestCase):
    """One host-side resolver over both stored blobs and bare local URIs.

    A finished-run reader hands it whatever an effector emitted -- an ``ReactionRef``
    or the raw mapping -- and gets a local path or ``None``, without re-deriving
    the content-addressed blob layout or the reaction URI grammar itself.
    """
    ...
    def test_resolves_stored_blob_from_content_addressed_store(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            source.write_bytes(b"reaction payload")
            store_root = root / "reactions"
            ref = FileReactionStore(store_root).put_file(kind="report", path=source)

            resolved = resolve_reaction_local_path(ref, store_root)
            self.assertIsNotNone(resolved)
            self.assertEqual(resolved.read_bytes(), b"reaction payload")
            # The raw mapping an effector emitted resolves identically to the ref.
            self.assertEqual(
                resolve_reaction_local_path(ref.model_dump(mode="json"), store_root),
                resolved,
            )
