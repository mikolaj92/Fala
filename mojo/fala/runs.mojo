...
struct RunLifecycle(Movable):
    ...
    @staticmethod
    def open(path: String) raises SQLiteError -> RunLifecycle:
        return RunLifecycle(path)

    def initialize(mut self) raises SQLiteError:
        self.journal.initialize()

    def _text(mut self, mut statement: Statement, index: Int) raises SQLiteError -> String:
        if statement.column_null(index):
            return String("")
        return statement.column_text(index)
    def _bind_nullable(mut self, mut statement: Statement, index: Int, value: String) raises SQLiteError:
        if value == "":
    ...
    def _read_record(mut self, mut statement: Statement) raises SQLiteError -> RunLifecycleRecord:
        if not statement.step():
            raise SQLiteError(code=1, message="Unknown run")
    ...
    def get_record(mut self, run_id: String) raises SQLiteError -> RunLifecycleRecord:
    ...
    def _read_run(mut self, mut statement: Statement) raises SQLiteError -> RunRow:
        if not statement.step():
            raise SQLiteError(code=1, message="Unknown run")
        return RunRow(
            id=self._text(statement, 0),
    ...
    def _find_run(mut self, run_id: String) raises SQLiteError -> RunRow:
        var statement = self.journal.db.query(
    ...
    def _command_exists(mut self, run_id: String, key: String) raises SQLiteError -> Bool:
        var statement = self.journal.db.query(
    ...
    def _transition(
        ...
        if run_id == "" or idempotency_key == "":
            raise SQLiteError(code=1, message="run lifecycle requires run_id and idempotency_key")
        ...
        if self._command_exists(run_id, idempotency_key):
            ...
            if not prior.step():
                raise SQLiteError(code=1, message="run lifecycle idempotency conflict")
        ...
        if not from_status.is_known() or not to_status.is_known():
            raise SQLiteError(code=1, message="Unknown run status")
        if not can_transition_run(from_status, to_status):
            raise SQLiteError(code=1, message="Invalid run status transition: " + current.status + " -> " + target)
        ...
        try:
            ...
            if self.journal.db.changes() != 1:
                raise SQLiteError(code=1, message="run changed concurrently")
            ...
            if not next.step():
                raise SQLiteError(code=1, message="unable to allocate event sequence")
            ...
            raise SQLiteError(code=1, message="run lifecycle transition failed")
    ...
    def create_result(
        ...
        actor: String = "",
    ) raises SQLiteError -> RunCreateResult:
        if run_id == "" or idempotency_key == "":
            raise SQLiteError(code=1, message="run creation requires run_id and idempotency_key")
        if not RunStatus(status).is_known():
            raise SQLiteError(code=1, message="Unknown run status: " + status)
        var cid = command_id if command_id != "" else idempotency_key
        var eid = cid + ":event"
        var replayed = False
        ...
        try:
            ...
            if not replayed:
                ...
                if existing.step():
                    raise SQLiteError(code=1, message="run already exists")
    ...
    def start(mut self, run_id: String, now: String, idempotency_key: String = "run.start") raises SQLiteError -> RunRow:
        return self._transition(run_id, "active", "run.start", "run.started", now, idempotency_key)

    def wait(mut self, run_id: String, now: String, idempotency_key: String = "run.wait") raises SQLiteError -> RunRow:
        return self._transition(run_id, "waiting", "run.wait", "run.waiting", now, idempotency_key)

    def complete(mut self, run_id: String, now: String, idempotency_key: String = "run.complete") raises SQLiteError -> RunRow:
        return self._transition(run_id, "completed", "run.complete", "run.completed", now, idempotency_key)

    def fail(mut self, run_id: String, now: String, idempotency_key: String = "run.fail") raises SQLiteError -> RunRow:
        return self._transition(run_id, "failed", "run.fail", "run.failed", now, idempotency_key)

    def request_cancel(mut self, run_id: String, now: String, idempotency_key: String = "run.cancel.request", reason: String = "cancel_requested", reason_present: Bool = True, actor: String = "") raises SQLiteError -> RunRow:
    ...
    def cancel(mut self, run_id: String, now: String, idempotency_key: String = "run.cancel", reason: String = "cancelled", actor: String = "") raises SQLiteError -> RunRow:
        return self._transition(run_id, "cancelled", "run.cancel", "run.cancelled", now, idempotency_key, reason=reason, actor=actor)

    def timeout(mut self, run_id: String, now: String, idempotency_key: String = "run.timeout", reason: String = "timed_out") raises SQLiteError -> RunRow:
