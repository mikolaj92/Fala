...
def put_bridge_delivery(mut store: NativeDomainStore, row: BridgeDelivery) raises -> BridgeDelivery:
    if not row.is_valid():
        raise Error(String(SQLiteError(code=1, message="domain store: invalid bridge delivery")))
...
def enqueue_bridge_delivery(
    ...
    if delivery_key == "": raise Error(String(SQLiteError(code=1, message="domain store: bridge enqueue idempotency key must not be empty")))
    ...
    if not delivery.is_valid() or delivery.status != "pending":
        raise Error(String(SQLiteError(code=1, message="domain store: bridge enqueue requires a valid pending delivery")))
    ...
    if delivery.updated_at == "": raise Error(String(SQLiteError(code=1, message="domain store: bridge enqueue updated_at must not be empty")))
    ...
    try:
        ...
        raise Error(String(SQLiteError(code=1, message="domain store: bridge enqueue payload is invalid JSON")))
    ...
    try:
        ...
        if existing.step():
            ...
            if stored.command_type != "bridge.outbox.enqueue" or stored.actor != actor or stored.correlation_id != correlation_id or stored.causation_id != causation_id or stored.created_at != delivery.updated_at or not _bridge_json_equal(stored.payload, command_payload):
                raise Error(String(SQLiteError(code=1, message="domain store: bridge idempotency conflict")))
            ...
            if not lookup.step() or lookup.column_text(0) == "":
                lookup.close(); raise Error(String(SQLiteError(code=1, message="domain store: bridge enqueue command identity is invalid")))
        ...
        if delivery.budget.spawned_runs_limited and not target_seen:
            ...
            if spawned.step() and spawned.column_int(0) >= delivery.budget.spawned_runs:
                spawned.close(); raise Error(String(SQLiteError(code=1, message="domain store: bridge spawned_runs budget exhausted")))
            spawned.close()
        ...
        if impulse_exists and (store._text(impulse_check,0) != delivery.impulse.run_id or store._text(impulse_check,1) != delivery.impulse.impulse_type or store._text(impulse_check,2) != delivery.impulse.payload or store._text(impulse_check,3) != delivery.impulse.metadata or store._text(impulse_check,4) != delivery.impulse.created_at or store._text(impulse_check,5) != delivery.impulse.updated_at):
            impulse_check.close(); raise Error(String(SQLiteError(code=1, message="domain store: bridge impulse idempotency conflict")))
        impulse_check.close()
        ...
        store.db.rollback(); raise Error(String(SQLiteError(code=1, message="domain store: bridge enqueue failed: " + String(err))))
def list_bridge_deliveries(mut store: NativeDomainStore, run_id: String) raises -> List[String]:
...
def _validate_bridge_refs(row: BridgeDelivery, table: String = "bridge_outbox") raises:
    ...
    if table != "bridge_inbox" and table != "bridge_outbox":
        raise Error(String(SQLiteError(code=1, message="domain store: invalid bridge table")))
    if table == "bridge_inbox":
        if row.target.run_id != row.run_id:
            raise Error(String(SQLiteError(code=1, message="domain store: bridge target run mismatch")))
        return
    if row.source.run_id != row.run_id:
        raise Error(String(SQLiteError(code=1, message="domain store: bridge source run mismatch")))
    if row.impulse.run_id != row.run_id:
        raise Error(String(SQLiteError(code=1, message="domain store: bridge impulse run mismatch")))
    if not (row.event_ref.runtime.id == "runtime" and row.event_ref.run_id == "run" and row.event_ref.event_id == "" and row.event_ref.sequence == 0):
        if row.event_ref.run_id != row.run_id:
            raise Error(String(SQLiteError(code=1, message="domain store: bridge event run mismatch")))
def put_inbox_delivery(mut store: NativeDomainStore, row: BridgeDelivery) raises -> BridgeDelivery:
    if not row.is_valid():
        raise Error(String(SQLiteError(code=1, message="domain store: invalid bridge delivery")))
    _validate_bridge_refs(row, "bridge_inbox")
    store._require_run(row.run_id)
    return _upsert_bridge_row(store, "bridge_inbox", row)
def _validate_bridge_budget(row: BridgeDelivery, next_attempt: Bool = True) raises:
    if row.budget.runtime_hops_limited and row.budget.runtime_hops < 1:
        raise Error(String(SQLiteError(code=1, message="domain store: bridge runtime hop budget exhausted")))
    if row.budget.impulse_count_limited and row.budget.impulse_count < 1:
        raise Error(String(SQLiteError(code=1, message="domain store: bridge impulse budget exhausted")))
    if next_attempt and row.budget.attempts_limited and row.attempts + 1 > row.budget.attempts:
        raise Error(String(SQLiteError(code=1, message="domain store: bridge attempts budget exhausted")))

def _normalize_imported_impulse(row: BridgeDelivery) raises -> Impulse:
    var metadata_value: Value
    try:
        metadata_value = Value(parse_string=row.impulse.metadata)
    except err:
        raise Error(String(SQLiteError(code=1, message="domain store: imported impulse metadata must be a JSON object")))
    ...
    try:
        ...
        raise Error(String(SQLiteError(code=1, message="domain store: imported impulse metadata serialization failed")))
...
def import_bridge_delivery(mut store: NativeDomainStore, row: BridgeDelivery, idempotency_key: String = "") raises -> BridgeDelivery:
    ...
    if delivery_key == "": raise Error(String(SQLiteError(code=1, message="domain store: bridge import idempotency key must not be empty")))
    ...
    if imported.updated_at == "": raise Error(String(SQLiteError(code=1, message="domain store: bridge import updated_at must not be empty")))
    return import_inbox_delivery(store, imported)
def import_inbox_delivery(mut store: NativeDomainStore, row: BridgeDelivery) raises -> BridgeDelivery:
    """Atomically import an inbox row, consuming bridge delivery budget."""
    if not row.is_valid() or row.status != "imported":
        raise Error(String(SQLiteError(code=1, message="domain store: inbox import requires valid imported delivery")))
    _validate_bridge_refs(row, "bridge_inbox")
    store._require_run(row.run_id)
    ...
    try:
        ...
        if collision.step():
            if store._text(collision, 0) != imported_impulse.impulse_type or store._text(collision, 1) != imported_impulse.payload or store._text(collision, 2) != imported_impulse.metadata or store._text(collision, 3) != imported_impulse.created_at or store._text(collision, 4) != imported_impulse.updated_at:
                collision.close()
                raise Error(String(SQLiteError(code=1, message="domain store: bridge impulse idempotency conflict")))
        collision.close()
        ...
        if has_existing and not bridge_status_transition_allowed(prior_status, row.status):
            raise Error(String(SQLiteError(code=1, message="domain store: invalid bridge status transition")))
        ...
        try:
            ...
            raise Error(String(SQLiteError(code=1, message="domain store: bridge budget exhausted")))
        ...
        if has_existing:
            ...
            if store.db.changes() != 1: raise Error(String(SQLiteError(code=1, message="domain store: inbox import lost ownership")))
        else:
        ...
        if detail.find("idempotency conflict") >= 0 or detail.find("status transition") >= 0 or detail.find("budget") >= 0:
            raise err^
        raise Error(String(SQLiteError(code=1, message="domain store: inbox import failed")))
    return _get_bridge(store, "bridge_inbox", row.run_id, row.id)

def _bridge_json_equal(left: String, right: String) raises -> Bool:
    var lhs = left
    var rhs = right
    if lhs == "": lhs = "null"
    if rhs == "": rhs = "null"
    try:
        ...
        raise Error(String(SQLiteError(code=1, message="domain store: invalid bridge JSON")))
def _record_bridge_journal(mut store: NativeDomainStore, run_id: String, command_id: String, command_type: String, payload: String, event_type: String, created_at: String, impulse_id: String = "") raises:
    ...
    if command_id == "" or command_type == "" or payload == "" or event_type == "" or created_at == "":
        raise Error(String(SQLiteError(code=1, message="domain store: bridge journal fields must not be empty")))
    ...
    if prior_event.step():
        ...
        if prior_type != event_type or prior_impulse != impulse_id or prior_command != stored_id or not _bridge_json_equal(prior_payload, payload):
            raise Error(String(SQLiteError(code=1, message="domain store: bridge event idempotency conflict")))
        return
    prior_event.close()
    ...
    if not next.step():
        next.close()
        raise Error(String(SQLiteError(code=1, message="domain store: bridge event sequence unavailable")))
...
def _bridge_operation_payload(delivery_id: String, status: String, attempts: Int = -1) -> String:
    ...
    payload += "}"
    return payload
def _upsert_bridge_row(mut store: NativeDomainStore, table: String, row: BridgeDelivery) raises -> BridgeDelivery:
    _validate_bridge_refs(row, table)
    if row.attempts < 0:
        raise Error(String(SQLiteError(code=1, message="domain store: invalid bridge attempts")))
    if table != "bridge_inbox" and table != "bridge_outbox":
        raise Error(String(SQLiteError(code=1, message="domain store: invalid bridge table")))
    var source_json = row.source.to_json()
    var target_json = row.target.to_json()
    ...
    try:
        ...
        if existing.step():
            ...
            existing.close()
            if not same_payload:
                store.db.rollback(); raise Error(String(SQLiteError(code=1, message="domain store: bridge idempotency conflict")))
            if not bridge_status_transition_allowed(prior_status, row.status):
                store.db.rollback(); raise Error(String(SQLiteError(code=1, message="domain store: invalid bridge status transition")))
        ...
        store.db.rollback()
        raise Error(String(SQLiteError(code=1, message="domain store: bridge delivery write failed")))
    if failure == "conflict":
        raise Error(String(SQLiteError(code=1, message="domain store: bridge idempotency conflict")))
...
def _read_bridge(mut stmt: Statement) raises -> BridgeDelivery:
    if not stmt.step():
        stmt.close()
        raise Error(String(SQLiteError(code=1, message="domain store: bridge delivery not found")))
    ...
    stmt.close()
    return row^
def list_bridge_inbox(mut store: NativeDomainStore, run_id: String, status: String = "", limit: Int = -1) raises -> List[String]:
    store._require_run(run_id)
...
def _get_bridge(mut store: NativeDomainStore, table: String, run_id: String, delivery_id: String) raises -> BridgeDelivery:
    if table != "bridge_inbox" and table != "bridge_outbox": raise Error(String(SQLiteError(code=1, message="domain store: invalid bridge table")))
    store._require_run(run_id)
...
def get_outbox_delivery(mut store: NativeDomainStore, run_id: String, delivery_id: String) raises -> BridgeDelivery:
    return _get_bridge(store, "bridge_outbox", run_id, delivery_id)
def get_inbox_delivery(mut store: NativeDomainStore, run_id: String, delivery_id: String) raises -> BridgeDelivery:
    return _get_bridge(store, "bridge_inbox", run_id, delivery_id)
def list_bridge_records(mut store: NativeDomainStore, table: String, run_id: String, status: String = "", limit: Int = -1) raises -> List[BridgeDelivery]:
    if limit < -1: raise Error(String(SQLiteError(code=1, message="domain store: limit must be non-negative")))
    if table != "bridge_inbox" and table != "bridge_outbox": raise Error(String(SQLiteError(code=1, message="domain store: invalid bridge table")))
    store._require_run(run_id)
...
def _read_bridge_current(mut stmt: Statement) raises -> BridgeDelivery:
...
def list_outbox_records(mut store: NativeDomainStore, run_id: String, status: String = "", limit: Int = -1) raises -> List[BridgeDelivery]:
    return list_bridge_records(store, "bridge_outbox", run_id, status, limit)
def list_inbox_records(mut store: NativeDomainStore, run_id: String, status: String = "", limit: Int = -1) raises -> List[BridgeDelivery]:
    return list_bridge_records(store, "bridge_inbox", run_id, status, limit)
def transition_bridge_delivery(mut store: NativeDomainStore, table: String, run_id: String, delivery_id: String, to_status: String, updated_at: String, idempotency_key: String = "") raises -> BridgeDelivery:
    if to_status != "pending" and to_status != "delivered" and to_status != "imported" and to_status != "failed":
        raise Error(String(SQLiteError(code=1, message="domain store: invalid bridge status")))
    if updated_at == "": raise Error(String(SQLiteError(code=1, message="domain store: bridge updated_at must not be empty")))
    var current = _get_bridge(store, table, run_id, delivery_id)
    if not bridge_status_transition_allowed(current.status, to_status): raise Error(String(SQLiteError(code=1, message="domain store: invalid bridge status transition")))
    ...
    try:
        if current.status != to_status:
            ...
            if store.db.changes() != 1: raise Error(String(SQLiteError(code=1, message="domain store: bridge transition lost ownership")))
        if idempotency_key != "":
...
def claim_bridge_delivery(mut store: NativeDomainStore, table: String, run_id: String, delivery_id: String, updated_at: String, idempotency_key: String = "") raises -> BridgeDelivery:
    return transition_bridge_delivery(store, table, run_id, delivery_id, "pending", updated_at, idempotency_key)
def deliver_bridge_delivery(mut store: NativeDomainStore, table: String, run_id: String, delivery_id: String, updated_at: String, idempotency_key: String = "") raises -> BridgeDelivery:
    if updated_at == "": raise Error(String(SQLiteError(code=1, message="domain store: bridge updated_at must not be empty")))
    var current = _get_bridge(store, table, run_id, delivery_id)
    ...
    try:
        ...
        if store.db.changes() != 1: raise Error(String(SQLiteError(code=1, message="domain store: bridge delivery lost ownership")))
        if idempotency_key != "":
        ...
        store.db.rollback(); raise err^
    return _get_bridge(store, table, run_id, delivery_id)
def retry_bridge_delivery(mut store: NativeDomainStore, table: String, run_id: String, delivery_id: String, updated_at: String, idempotency_key: String = "") raises -> BridgeDelivery:
    if table != "bridge_inbox" and table != "bridge_outbox": raise Error(String(SQLiteError(code=1, message="domain store: invalid bridge table")))
    if updated_at == "": raise Error(String(SQLiteError(code=1, message="domain store: bridge updated_at must not be empty")))
    var current = _get_bridge(store, table, run_id, delivery_id)
    if current.status != "pending": raise Error(String(SQLiteError(code=1, message="domain store: invalid bridge status transition")))
    var next_attempts = current.attempts + 1
    if idempotency_key != "":
    ...
    try:
        ...
        if store.db.changes() != 1: raise Error(String(SQLiteError(code=1, message="domain store: bridge retry lost ownership")))
