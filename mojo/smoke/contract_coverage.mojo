...
def main() raises:
    ...
    Path(optout).write_text("{\"id\":\"missing\",\"version\":\"2\",\"correlation_paths\":[{\"id\":\"ship\",\"effectors\":[{\"id\":\"step\",\"adapter\":{\"kind\":\"manual_homeostat\"}}],\"terminals\":[{\"id\":\"done\",\"source_effector\":\"step\",\"status\":\"succeeded\",\"output_schema\":{\"type\":\"object\",\"required\":[\"ok\"],\"properties\":{\"ok\":{\"type\":\"boolean\"}}}}]}]}")
    var optout_report = graph_validate(optout)
    expect(optout_report.find("\"valid\":false") >= 0 and optout_report.find("manifest.missing") >= 0 and optout_report.find("output_schema") >= 0, "missing output contract must fail closed")

    Path(unproven).write_text("{\"id\":\"unproven\",\"version\":\"2\",\"correlation_paths\":[{\"id\":\"ship\",\"effectors\":[{\"id\":\"step\",\"output_schema\":{\"type\":\"object\",\"required\":[\"route\"],\"properties\":{\"route\":{\"type\":\"string\"}}},\"adapter\":{\"kind\":\"manual_homeostat\"}}],\"terminals\":[{\"id\":\"done\",\"source_effector\":\"step\",\"status\":\"succeeded\",\"output_schema\":{\"type\":\"object\",\"required\":[\"ok\"],\"properties\":{\"ok\":{\"type\":\"boolean\"}}}}]}]}")
