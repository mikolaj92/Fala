...
def main() raises:
    ...
    var message = result_message("request", "source", 1, '{"route":"ready","artifact":"local.txt"}')
    ...
    var with_telemetry = message.replace('"message_kind":', '"adapter":{"returncode":0},"message_kind":')
    _ = journal.complete_process("contract", "source", "test", "2026-01-01T00:00:02Z", with_telemetry)
    expect(journal.get_process("contract", "source").status == "succeeded", "FEP values must satisfy domain schema")
    journal.close()
