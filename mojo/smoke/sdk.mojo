...
def main() raises:
    ...
    _check(result.find("\"contract_id\":\"echo-output\"") >= 0 and result.find("\"contract_version\":\"1\"") >= 0, "sdk output echoes the required contract pair")
    var contracted_request = request_message("parent", "echo", "echo", "{\"ok\":true}", "{}", "other-output", "2")
    var contracted_result = output(contracted_request, "{\"ok\":true}")
    _check(contracted_result.find("\"contract_id\":\"other-output\"") >= 0 and contracted_result.find("\"contract_version\":\"2\"") >= 0, "sdk output echoes the request contract pair")
