...
def main() raises:
    ...
    _check(_has(doctor_package_removed, "argument_error") and not _has(doctor_package_removed, "native_boundary") and not _has(doctor_package_removed, "YAML"), "doctor --package is an unknown option")
    ...
    var inspected_relation = dispatch_native_command("impulse-relations inspect --db " + path + " --run-id cli-run --relation-id rel-source")
    ...
    var missing_relation = dispatch_native_command("impulse-relations inspect --db " + path + " --run-id cli-run --relation-id missing-relation")
    ...
    var homeostat_clock_boundary = dispatch_native_command("homeostats open --db " + path + " --run-id cli-run --homeostat-id homeostat-clock --kind manual --values-json '{}' --metadata-json '{}'")
    ...
    var homeostat_bad_value = dispatch_native_command("homeostats complete --db " + path + " --run-id cli-run --homeostat-id homeostat-demo --value malformed")
    _check(_has(homeostat_bad_value, "Invalid value 'malformed'; expected key=value"), "domain homeostat value format")
    var homeostat_bad_values_json = dispatch_native_command("homeostats open --db " + path + " --run-id cli-run --homeostat-id homeostat-json --kind manual --values-json [] --metadata-json '{}'")
    _check(_has(homeostat_bad_values_json, "invalid_json"), "domain homeostat values must be object JSON")
    var homeostat_bad_metadata_json = dispatch_native_command("homeostats open --db " + path + " --run-id cli-run --homeostat-id homeostat-json --kind manual --values-json '{}' --metadata-json []")
    _check(_has(homeostat_bad_metadata_json, "invalid_json"), "domain homeostat metadata must be object JSON")
    var homeostat_reopen = dispatch_native_command("homeostats reopen --db " + path + " --run-id cli-run --homeostat-id homeostat-demo --process-id process-demo --now 2026-01-01T00:00:06Z")
    _check(not _has(homeostat_reopen, "unsupported_command") and _has(homeostat_reopen, "storage_error"), "homeostat reopen is dispatched through process validator")
    var missing_reaction_record = dispatch_native_command("reactions record --db " + path + " --run-id cli-run --reaction-root /tmp/fala-reactions --path /tmp/missing-reaction --kind text/plain --now 2026-01-01T00:00:00Z")
    _check(_has(missing_reaction_record, "argument_error") and _has(missing_reaction_record, "reaction path must be a file"), "reaction record is dispatched through validators")
    var homeostat_repeat_values = dispatch_native_command("homeostats complete --db " + path + " --run-id cli-run --homeostat-id homeostat-demo --value score=1 --value score=2 --value note=done")
    var homeostat_values_json = _homeostat_domain_values("homeostats complete --db " + path + " --run-id cli-run --homeostat-id homeostat-demo --value score=1 --value score=2 --value note=done")
    ...
    var missing_homeostat_db = dispatch_native_command("homeostats open --db")
