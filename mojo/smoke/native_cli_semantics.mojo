...
def main() raises:
    ...
    var homeostat_bad_values_json = dispatch_native_command("homeostats open --db " + path + " --run-id cli-run --homeostat-id homeostat-json --kind manual --values [] --metadata '{}'")
    _check(_has(homeostat_bad_values_json, "invalid_json"), "domain homeostat values must be object JSON")
    var homeostat_bad_metadata_json = dispatch_native_command("homeostats open --db " + path + " --run-id cli-run --homeostat-id homeostat-json --kind manual --values '{}' --metadata []")
