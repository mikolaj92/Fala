...
if [[ "$need_host" == "1" ]]; then
  ...
  fixture_source="$root/mojo/smoke/native_effector_fixture.c"
  fixture_builder="$root/tools/build_native_effector_fixture.sh"
  if [[ ! -x "$fixture" || "$fixture_source" -nt "$fixture" || "$fixture_builder" -nt "$fixture" ]]; then
