...
| SQLite backend | PARTIAL | Reference backend exists with transactional run creation, carrier acceptance, carrier type registration, and carrier relation recording; append-only commands/events; processes, gates, projections, artifacts, bridge inbox/outbox; schema tracking; and run-scoped writes reject unknown runs. Further hardening can continue around broader replay guarantees. |
...
| Runtime backend boundary | PARTIAL | `RuntimeBackend` covers the current runtime operations, including transactional `create_run`, `accept_carrier`, `register_carrier_type`, and `record_carrier_relation`; backend conformance should continue expanding with every new mutation. |
