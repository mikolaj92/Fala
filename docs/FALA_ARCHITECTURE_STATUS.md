...
| SQLite backend | PARTIAL | Reference backend exists with transactional run creation, carrier acceptance, carrier type registration, carrier relation recording, observation recording, artifact recording, and process scheduling; append-only commands/events; process transitions, gates, projections, bridge inbox/outbox; schema tracking; and run-scoped writes reject unknown runs. Further hardening can continue around broader replay guarantees. |
