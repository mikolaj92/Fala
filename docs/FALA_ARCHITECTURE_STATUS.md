...
| Effector adapters | PARTIAL | Core: `python_function`, `subprocess`, `manual_homeostat` (+ Mojo `native_function`). `fala_runtime` / RuntimePool **removed** from product surface. Process host is merge-gate for Mojo land. |
...
| Multi-Fala composition | REFRAMED | Nested = process host + separate journals + optional bridge envelope. Fleet/pools **removed** from product. See FALA_HOST_AND_COMPOSITION. |
