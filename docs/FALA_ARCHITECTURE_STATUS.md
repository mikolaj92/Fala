...
| SQLite backend | PARTIAL | Reference backend exists with transactional run creation, append-only commands/events, carriers, processes, gates, projections, artifacts, bridge inbox/outbox, schema tracking, and run-scoped writes reject unknown runs. Further hardening can continue around broader replay guarantees. |
...
| Runtime backend boundary | PARTIAL | `RuntimeBackend` covers the current runtime operations, including transactional `create_run`; backend conformance should continue expanding with every new mutation. |
