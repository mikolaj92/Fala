...
| SQLite backend | PARTIAL | Reference backend exists with append-only commands/events, carriers, processes, gates, projections, artifacts, bridge inbox/outbox, and schema tracking. Further hardening can continue around broader replay guarantees. |
...
| Commands and idempotency | PARTIAL | Runtime service mutations submit append-only commands with idempotency keys, including explicit `run.cancel`, and command logs are inspectable through the backend and CLI. Some low-level backend put methods remain for backend implementation and tests. |
