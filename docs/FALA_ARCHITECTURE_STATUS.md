...
| Commands and idempotency | PARTIAL | Runtime service mutations submit append-only commands with idempotency keys, and command logs are inspectable through the backend and CLI. Some low-level backend put methods remain for backend implementation and tests. |
