...
| State machines | PARTIAL | Run/process/gate statuses exist with transition checks for key paths, including terminal process retry/complete, wait-from-running, and create-only gate opening guards. More exhaustive transition matrix tests can still be added. |
