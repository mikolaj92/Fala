...
| State machines | PARTIAL | Run/process/gate statuses exist with transition checks for key paths, including run transition matrix coverage, process scheduling initial-status guards, terminal process retry/complete, wait-from-running, and create-only gate opening guards. More exhaustive gate transition matrix tests can still be added. |
