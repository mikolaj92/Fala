...
| State machines | PARTIAL | Run/process/gate statuses exist with transition checks for key paths, including run transition matrix coverage, process scheduling initial-status guards, terminal process retry/complete, wait-from-running, and gate complete/cancel/expire terminal transitions. More exhaustive process matrix tests can still be added. |
