...
| State machines | PARTIAL | Run/process/gate statuses exist with transition checks for key paths, including run transition matrix coverage, terminal process retry/complete, wait-from-running, and create-only gate opening guards. More exhaustive process and gate transition matrix tests can still be added. |
