...
def test_full_schema_contract_rejects_noncore_shape_before_mutation(tmp_path) -> None:
    ...
    with pytest.raises(RuntimeError, match="incompatible impulses|schema-v6 table"):
        fala.ensure_journal(db)
    with sqlite3.connect(db) as conn:
...
_FOREIGN_SHAPE = "CREATE TABLE runtime_events (run_id TEXT NOT NULL, sequence INTEGER NOT NULL, id TEXT PRIMARY KEY, event_type TEXT NOT NULL, payload TEXT NOT NULL, created_at TEXT NOT NULL)"


@pytest.mark.parametrize("mutation", ["minimal", "extra", "nullable", "wrong-pk"])
def test_ensure_journal_rejects_noncurrent_schema_and_leaves_it_unchanged(tmp_path, mutation) -> None:
    import fala
    ...
    elif mutation == "wrong-pk":
        sql = sql.replace("id TEXT PRIMARY KEY", "id TEXT")
    with sqlite3.connect(db) as conn:
        conn.execute(sql)
        conn.execute("INSERT INTO runtime_events (run_id,sequence,id,event_type,payload,created_at) VALUES ('foreign',1,'sentinel','type','{}','now')")
    with pytest.raises(RuntimeError, match="incompatible|missing|not current|initialization failed"):
        fala.ensure_journal(db)
    with sqlite3.connect(db) as conn:
